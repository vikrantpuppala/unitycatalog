...
public class CoordinatedCommitsHandler {
    ...
    public static void handleFirstCommit(Commit commit) {
        validateOnboardingCommit(commit);
        COMMIT_REPOSITORY.saveCommit(commit);
    }

    public static void handleBackfillOnlyCommit(String tableId, Long latestBackfilledVersion, CommitDAO firstCommit, CommitDAO lastCommit) {
    ...
    public static void handleReboardCommit() {
        // NO-OP since this would probably not be allowed.
    }

    public static void handleNormalCommit(Commit commit, CommitDAO firstCommit, CommitDAO lastCommit) {
        if (commit.getCommitInfo().getVersion() <= lastCommit.getCommitVersion()) {
            throw new RuntimeException("Commit version should be greater than the last commit version = " + lastCommit.getCommitVersion());
        }
        if (commit.getCommitInfo().getVersion() > lastCommit.getCommitVersion() + 1) {
            throw new RuntimeException("Commit version should be the next version after the last commit version = " + lastCommit.getCommitVersion());
        }
        if (commit.getLatestBackfilledVersion() != null && commit.getLatestBackfilledVersion() > lastCommit.getCommitVersion()) {
            throw new RuntimeException("Latest backfilled version cannot be greater than the last commit version = " + lastCommit.getCommitVersion());
        }
        ...
        if (commit.getCommitInfo().getVersion() - latestBackfilledVersion > MAX_NUM_COMMITS_PER_TABLE) {
            throw new RuntimeException("Max number of commits per table reached = " + MAX_NUM_COMMITS_PER_TABLE);
        }
        COMMIT_REPOSITORY.saveCommit(commit);
        if (latestBackfilledVersion >= firstCommit.getCommitVersion()) {
            COMMIT_REPOSITORY.backfillCommits(
    ...
    private static Long getLatestBackfilledVersion(Commit commit, CommitDAO firstCommit, CommitDAO lastCommit) {
        ...
        if (lastCommit.getIsBackfilledLatestCommit()) {
            if (!Objects.equals(firstCommit.getCommitVersion(), lastCommit.getCommitVersion())) {
                LOGGER.error("When the last commit is the latest backfilled commit, there should be exactly one commit");
                // This is recoverable
            }
            latestBackfilledVersion = lastCommit.getCommitVersion();
        } else {
            latestBackfilledVersion = Math.max(
